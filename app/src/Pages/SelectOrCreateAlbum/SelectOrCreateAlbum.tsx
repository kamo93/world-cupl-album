...
async function fetcherPost(
  url: string,
  { arg }: { arg: { name: string; email: string; albumTypeId: string } },
) {
  return await customFetch.post<AlbumsPostApiResponse>({
    url,
    body: { name: arg.name, email: arg.email, albumTypeId: arg.albumTypeId },
  });
}
...
function SelectOrCreateAlbum(): JSX.Element {
  ...
  const { trigger, isMutating } = useSWRMutation<
    AlbumsPostApiResponse,
    { error: string },
    "/api/album",
    { name: string; email: string; albumTypeId: string },
    AlbumsPostApiResponse
  >("/api/album", fetcherPost);
  ...
  return (
    <StyledContainer>
      ...
      <Section>
        <p>
          Para la creacion de tu album debes escoger el tipo de album y poner un
          nombre al mismo.
        </p>
      </Section>
      <StyledForm onSubmit={handlerOnSubmit}>
        <StyledFieldContainer>
          <Form.Field>
            <Form.Label htmlFor="albumTypeId">
              Selecciona el album que vas a completar
            </Form.Label>
            <Form.Control>
        ...
        <Form.Control>
          <Button submit fullwidth color="info" outlined loading={isMutating}>
            Confirmar
          </Button>
        </Form.Control>
      </StyledForm>
