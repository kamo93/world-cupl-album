...
const album: FastifyPluginAsyncJsonSchemaToTs = async (
  fastify,
): Promise<void> => {
  ...
  fastify.get<{ Params: FromSchema<typeof paramsAlbumSchema> }>(
    ...
    async (req, res) => {
      ...
      if (data?.length === 0) {
        return await res
          .code(404)
          .send({ message: `The album ${albumId} doesn't found.` });
      }
      if (error !== null) {
  ...
  fastify.put<{
    Params: FromSchema<typeof paramsAlbumSchema>;
    Body: FromSchema<typeof bodyPut>;
  }>("/albums/:albumId", { schema: putSchema }, async (req, res) => {
    const { albumId } = req.params;
    const { userEmail } = req.body;
    const { error: cleanSelectedError } = await fastify.supabase
      .from("album_members")
  ...
  const albumsXUserSchema: FastifySchema = {
    params: queryParamsAlbumsXUserSchema,
  };
        ...
        return {
          ...
          data: data.map(({ name, id, album_members }) => {
            return {
              name,
              id,
              selected: Array.isArray(album_members)
                ? album_members[0].selected
                : album_members?.selected,
            };
          }),
        };
      }
  ...
  fastify.get("/albums", async (req, rep) => {
    const { data, error } = await fastify.supabase
      .from("albums")
      .select("id, name");
    if (data?.length === 0) {
  ...
  fastify.get<{
    Querystring: FromSchema<typeof queryParamsAlbumPerEmailSchema>;
  }>("/album", { schema: schemaAlbumQuery }, async (req, res) => {
    ...
    if (data?.length === 0) {
      return await res
        .code(404)
        .send({ message: `Not users with the next email ${userEmail}` });
    }
    if (error !== null) {
      return await res.code(500).send({ message: error.hint });
    }
  ...
  const albumSyncSchema: FastifySchema = {
    querystring: querystringAlbumSyncSchema,
  };
  fastify.get<{ Querystring: FromSchema<typeof querystringAlbumSyncSchema> }>(
    ...
    async (connection, request) => {
      ...
      if (data !== null && data.length > 0) {
        ...
        channel = fastify.supabase.channel(`channel:album:${albumId}`, {
        ...
        channel
          ...
          .on("broadcast", { event: "update_album" }, async ({ payload }) => {
            fastify.log.info(
              { "msg-channel": payload },
              `channel ${payload.action as ALBUM_SYNC_ACTIONS_TYPES}`,
            );
            const { number, code, origin, action } = payload;
            // @typescript
            const stickersPerCode = stickers[code].figures.map(
              (sticker: Figure) => {
                if (sticker.value === number) {
                  let newValue: number;
                  if (ALBUM_SYNC_ACTIONS.remove === action) {
                    newValue =
                      sticker.repeat > 0 ? sticker.repeat - 1 : sticker.repeat; // just remove when its positive you can't own stickers
                  } else {
            ...
            connection.socket.send(
              JSON.stringify({
                data: { name: data[0].name, stickers },
                origin,
                action,
              }),
            );
      ...
      connection.socket.on("message", async (message) => {
        ...
        await channel.send({
          payload: { code, number, origin, action },
          type: "broadcast",
          event: "update_album",
        });
      });
