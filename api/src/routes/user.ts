...
const users: FastifyPluginAsyncJsonSchemaToTs = async (
  fastify,
  opts,
): Promise<void> => {
  ...
  fastify.post<{ Body: FromSchema<typeof bodyUserSchema> }>(
    ...
    async function (request, reply) {
      ...
      if (data?.length === 0) {
        await reply
          .code(404)
          .send({ message: `No user could add with ${request.body.email}.` });
      }
      if (error !== null) {
  ...
  const schemaGetUsersXAlbumId: FastifySchema = {
    params: paramsGetUsersXAlbumId,
  };
